// ==Content Script for Bilibili Video Filter Extension==
// This is the content script for the browser extension version of Bilibili-Video-Filter.
...
(function () {
    ...
    class ConfigManager {
        constructor() {
            ...
            this.defaultConfig = {
                ...
                category: {
                    enabled: true,
                    blacklist: ['番剧', '直播', '国创', '综艺', '课堂', '电影', '电视剧', '纪录片', '漫画']
                },
                ad: true,
                live: true,
                continuousBlock: false,
                floatBtnPosition: { x: 30, y: 100 },
                darkMode: false
            };
        ...
        async loadConfig() {
            try {
                ...
                if (this.legacyKeys.some(key => key in result)) {
                    this.migrateLegacyConfig(result);
                ...
                Object.keys(result).forEach(key => {
                    ...
                    current[keys[keys.length - 1]] = result[key];
                });
            } catch (error) {
                console.warn('[B站过滤插件] 配置加载失败，使用默认配置:', error);
                this.config = { ...this.defaultConfig };
            }
        }
        migrateLegacyConfig(result) {
            if ('videoEnabled' in result) {
                this.config.video.enabled = result.videoEnabled;
                delete result.videoEnabled;
            }
            if ('videoBlacklist' in result) {
                this.config.video.blacklist = result.videoBlacklist || [];
                delete result.videoBlacklist;
            }
            if ('categoryEnabled' in result) {
                this.config.category.enabled = result.categoryEnabled;
                delete result.categoryEnabled;
            }
            if ('categoryBlacklist' in result) {
                this.config.category.blacklist = result.categoryBlacklist || this.defaultConfig.category.blacklist;
                delete result.categoryBlacklist;
            }
            if ('adEnabled' in result) {
                this.config.ad = result.adEnabled;
                delete result.adEnabled;
            }
            if ('liveEnabled' in result) {
                this.config.live = result.liveEnabled;
                delete result.liveEnabled;
            }
        ...
        async setValue(key, value) {
            try {
                ...
                console.warn('[B站过滤插件] 配置保存失败:', error);
            }
        }
        async reset() {
            try {
                ...
                console.warn('[B站过滤插件] 配置重置失败:', error);
            }
        }
        async saveConfig() {
            const toSave = {};
            Object.keys(this.config).forEach(key => {
                const fullKey = typeof key === 'string' ? key : Object.keys(key)[0]; // Handle nested
                // Flatten config to storage format
                const flatten = (obj, prefix = '') => {
                    Object.keys(obj).forEach(k => {
                        const fullPath = prefix ? `${prefix}.${k}` : k;
                        if (typeof obj[k] === 'object' && obj[k] !== null) {
                            flatten(obj[k], fullPath);
                        } else {
                            toSave[fullPath] = obj[k];
                        }
                    });
                };
                flatten(this.config);
    ...
    function createMainPanel() {
        ...
        `;
        // Event listeners
        panel.querySelector('.theme-toggle-btn').addEventListener('click', async (e) => {
            const body = document.body;
            const currentTheme = body.dataset.theme === 'dark';
            const newTheme = !currentTheme;
            body.dataset.theme = newTheme ? 'dark' : 'light';
            await configManager.setValue('darkMode', newTheme);
            e.currentTarget.innerHTML = newTheme ? ICONS.sun : ICONS.moon;
        });
        panel.querySelector('#continuous-block').addEventListener('change', async (e) => {
            await configManager.setValue('continuousBlock', e.target.checked);
        ...
        panel.querySelector('#video-enabled').addEventListener('change', async (e) => {
            await configManager.setValue('video.enabled', e.target.checked);
            runAllBlockers();
        });
        panel.querySelector('#category-enabled').addEventListener('change', async (e) => {
            await configManager.setValue('category.enabled', e.target.checked);
            runAllBlockers();
        });
        panel.querySelector('#ad-enabled').addEventListener('change', async (e) => {
            await configManager.setValue('ad', e.target.checked);
            runAllBlockers();
        });
        panel.querySelector('#live-enabled').addEventListener('change', async (e) => {
            await configManager.setValue('live', e.target.checked);
            runAllBlockers();
        });
        ...
        panel.querySelector('#reset-config').addEventListener('click', async () => {
        ...
        const updateList = async () => {
            ...
            list.querySelectorAll('.delete-btn').forEach(btn => {
                btn.addEventListener('click', async () => {
                    ...
                    await configManager.setValue(`${type}.blacklist`, newList);
                    updateList();
                    runAllBlockers();
                });
            });
    ...
    function createDraggableFloatBtn(mainPanelElement) {
        ...
        const handleEnd = async () => {
            ...
            if (hasMoved) {
                const rect = btn.getBoundingClientRect();
                await configManager.setValue('floatBtnPosition', { x: rect.left, y: window.innerHeight - rect.bottom });
            } else {
